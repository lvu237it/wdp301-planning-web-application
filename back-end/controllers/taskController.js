...
exports.assignTask = async (req, res, next) => {
  try {
    ...
    if (!user || !user.email) {
      return res.status(404).json({
        status: 'fail',
        message: 'User not found or email is not valid',
      });
    }
...
exports.updateTask = async (req, res) => {
  try {
    ...
    if (hasSignificantChanges && task.assignedTo) {
      ...
      if (
        changeDetails.includes('ngày bắt đầu') ||
        changeDetails.includes('ngày kết thúc')
      ) {
        try {
          const assignedUser = await User.findById(task.assignedTo).select(
            'email name'
          ...
          if (assignedUser?.email) {
            ...
            const html = `
              ...
              <p>Thời gian của task bạn đang nhận <strong>"${
                task.title
            ...
            console.log(
              `Gửi mail thay đổi thời gian cho: ${assignedUser.email}`
            );
          } else {
            console.warn(
              'Không tìm thấy email của assignedUser, bỏ qua gửi mail'
            );
          }
        } catch (mailError) {
    ...
    res
      .status(500)
      .json({ status: 'error', message: 'Error while updating task' });
  }
};
...
exports.deleteTask = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      status: 'error',
      message: error.message || 'Error while deleting task',
    });
  } finally {
    session.endSession();
  }
...
exports.updateChecklistItem = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        status: 'fail',
        message: 'Index checklist item is not valid',
      });
    }
    ...
    res.status(500).json({
      status: 'error',
      message: 'Error while updating checklist item',
    });
  }
};
...
exports.getTasksByUser = async (req, res) => {
  try {
    ...
    res.status(500).json({
      status: 'error',
      message: 'Error while fetching tasks by user',
    });
  }
};
