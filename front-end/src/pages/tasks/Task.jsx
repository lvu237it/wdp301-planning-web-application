...
const TaskModal = ({ isOpen, task, onClose, onUpdate }) => {
  ...
  const handleSaveDesc = async () => {
    try {
      ...
      const res = await fetch(`${apiBaseUrl}/task/updateTask/${task._id}`, {
        method: 'PUT',
        credentials: 'include',
        headers: {
      ...
      <div className='task-modal' onClick={(e) => e.stopPropagation()}>
        ...
        <div className='task-modal-header'>
          <select className='task-modal-list-dropdown'>
            <option value={task.listId}>{task.listTitle || 'Danh sách'}</option>
          </select>
        ...
        <div className='task-modal-body'>
          ...
          <div className='task-modal-actions'>
            ...
            <button className='btn-action'>
              <i className='fas fa-list'></i> Việc cần làm
            </button>
            <button className='btn-action'>
              <i className='fas fa-paperclip'></i> Đính kèm
            </button>
          </div>
